...
run()
{
  ...
  tests_passed += check_map<int>(
    *allocator, toto, algorep::DataType::INT,
    algorep::callback::Type::I_POW,
    int_comp
  );

  std::vector<float> toto2({5.3, 0.01, 0.006, 1039393.9, 0.0});
  tests_passed += check_map<float>(
    *allocator, toto2, algorep::DataType::FLOAT,
    algorep::callback::Type::F_POW,
    float_comp
  );

  std::vector<double> toto3({5.30000, 0.000000001, 4981914.1111, 0.09});
  tests_passed += check_map<double>(
    *allocator, toto3, algorep::DataType::DOUBLE,
    algorep::callback::Type::D_POW,
    float_comp
  );
  ...
  tests_passed += check_map<unsigned long>(
    *allocator, toto4, algorep::DataType::ULONG,
    algorep::callback::Type::UL_POW,
    int_comp
  );
