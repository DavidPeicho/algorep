...
run()
{
  ...
  std::vector<double> toto({0.0001, 1.0, 1.1, 1.4, 0.76});
  tests_passed +=
      check_reduce<double>(*allocator, toto, ReduceID::D_SUM, 0.0, float_comp);
  tests_passed += check_reduce<double>(*allocator, toto, ReduceID::D_SUM,
                                       9182.5, float_comp);
  std::vector<int> toto2(
      {0, 0, 0, 1, 4, 9, -1293, 1000, 292, 1, 100, -99, 13, -10, 15});
  tests_passed +=
      check_reduce<int>(*allocator, toto2, ReduceID::I_SUM, 0, int_comp);
  std::vector<unsigned short> toto3({100, 4024, 10000, 500, 111});
  tests_passed += check_reduce<unsigned short>(*allocator, toto3,
                                               ReduceID::US_SUM, 1, int_comp);
  std::vector<unsigned long> toto4({1000000, 14842044, 193133, 0, 0, 0, 0, 0, 0,
                                    101933, 1119203, 11222, 11});
